...
for idx_start=1:sStart    
  ...
  if Sim.cal_onoe
      ...
      plot(plot_col_onoe, plot_thr_onoe, ['r' symbol_plot( rem(idx_snr, len_symbol) ) style_plot(1+(1-1)*2) style_plot(2+(1-1)*2)], 'LineWidth', line_width);
      hold on;      grid on;
  ...
  if Sim.cal_aarf
      ...
      plot(plot_col_aarf, plot_thr_aarf, ['g' symbol_plot( rem(idx_snr, len_symbol) ) style_plot(1+(1-1)*2) style_plot(2+(1-1)*2)], 'LineWidth', line_width);
      hold on;      grid on;
      xlabel('collided packets');      ylabel('Throughput');
legend(['SNR: ' num2str(Phy.snr_set(1))] , ['SNR: ' num2str(Phy.snr_set(2))], ['SNR: ' num2str(Phy.snr_set(3))] , ['SNR: ' num2str(Phy.snr_set(4))]);
      set(gca, 'FontSize', font_size, 'LineWidth', line_width);     
