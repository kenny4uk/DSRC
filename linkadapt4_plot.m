...
font_size=13; line_width=1.4;

for idx_period=1:sPeriodset
for idx_snr=1:sSnr    
for idx_start=1:1 % sStart    
  ...
  if Sim.cal_sample
      fig_org=100;
      figure(fig_org+1+idx_start);
      plot(Sim.node_set, plot_thr_sample, ['k' symbol_plot( rem(idx_snr, len_symbol) ) style_plot(1+(1-1)*2) style_plot(2+(1-1)*2)], 'LineWidth', line_width); 
      hold on;      grid on;
