...
for idx_start=1:sStart    
  ...
  if Sim.cal_onoe
      ...
      plot(Sim.node_set, plot_thr_onoe, ['r' symbol_plot( rem(idx_snr, len_symbol) ) style_plot(1+(1-1
...
%grid on
      if bl_epssave==1;    eval(['print -deps ' epsname 'through-sim_onoe-node' num2str(max(Sim.node_set)) ...
              '-period' num2str(Onoe.period_set(idx_period)*10) '-start' num2str(Sta)*2) style_plot(2+(1-1)*2)], 'LineWidth', line_width); 
      hold on;      grid on;
      xlabel('Number of nodes');   ylabel('Throughput');rtrate_mode(idx_start)) ] ); end;       
  end
