...
[out] = access
%[out] = access2
[pathloss]= p_loss(cars,xpos,ypos);%propagation loss
[shadowloss] = shadow();
...
Sim.pk_basic=100;     % Total number of packets to be successfully sent per simulation
%Sim.node_set=[1 5:10:30];
% Sim.node_set=[1];
sNode=length(cars);
...
matname= [num2str(hh(1)) '-' num2str(hh(2)) '-'  num2str(hh(3)) '-'  num2str(hh(4)) '-'  num2str(hh(5))  '-linkadapt4'] ;
epsname=matname;
bl_matsave=1;
bl_epssave=1; 
...
% Phy.rate_mode=[5]; 
Phy.power=40; % normalized transmit power, 1 Watt.
...
for idx_start=1:sStart    
    ...
    Sim.n=cars(idx_node);                       % number of vehicles on network
  ...
  thr_mod_onoe_iter=zeros(1, iter_num);
  eneff_mod_onoe_iter=zeros(1,iter_num);
  col_mod_onoe_iter=zeros(1, iter_num);
  suc_mod_onoe_iter=zeros(1, iter_num);
...
%           disp(['current time is: ' num2str(curr_time(2)) '-'  num2str(curr_time(3)) '-'  num2str(curr_time(4)) '-'  num2str(curr_time(5))]);
          ...
          alg_sample();
          thr_sample_iter(idx_iter)=mobile.through;
          eneff_sample_iter(idx_iter)=mobile.energyeff;          
          col_sample_iter(idx_iter)=mobile.pk_col;
          suc_sample_iter(idx_iter)=mobile.pk_suc;
          per_sample_iter(idx_iter)=mobile.pk_per;          
